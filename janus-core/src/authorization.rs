...
mod tests {
    ...
    fn signed_license(now: DateTime<Utc>) -> (SignedLicense, String) {
        ...
        (
            SignedLicense { claims, signature: STANDARD.encode(signature.to_bytes()) },
    ...
    fn rejects_a_changed_module_even_with_valid_license_and_scope() {
        ...
        let engagement = Engagement {
            id: "eng_1".into(), organization_id: "org_1".into(), authorization_reference: "AUTH-1".into(),
            starts_at: now - chrono::Duration::hours(1), ends_at: now + chrono::Duration::hours(1), active: true,
            scope: EngagementScope { approved_assets: vec!["evidence-1".into()], approved_evidence_paths: vec![], approved_features: vec![LicensedFeature::Forensics] },
        };
        let certification = ModuleCertification {
            module_id: "forensics.hash".into(), module_sha256: "reviewed-hash".into(),
            status: CertificationStatus::ProductionApproved, required_feature: LicensedFeature::Forensics,
            reviewed_by: Some("reviewer".into()), reviewed_at: Some(now), notes: None,
        };
        let result = authorize_execution(ExecutionAuthorization {
            license: &license, license_public_key_base64: &public_key, engagement: &engagement,
            certification: &certification, module_sha256: "changed-hash", target_asset: "evidence-1", now,
        });
